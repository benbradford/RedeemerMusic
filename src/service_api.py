...
def service_api():
    ...
    service_email_details = _get_email_details(service, 'Service', 'email_status',
                                               RecipientsHelper().get_all_recipients())
    ppt_email_details = _get_email_details(service, 'Powerpoint', 'slides_email_status',
                                           RecipientsHelper().get_ppt_recipients())
    return render_template('service.html', service=service, service_email_params=service_email_details,
                           ppt_email_params=ppt_email_details, songs=songs_dao.get_all())


@app.route('/edit_service', methods=['GET'])
def edit_service_api():
    service = service_dao.get(extract_required_param('id'))
