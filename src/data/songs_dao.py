...
def _db_add_song(song):
    return _db_add_songs([song])


def _db_update_song(song):
    with DbAccess() as cur:
        cur.execute("""
    ...
    def update(self, song_name, song_creation_data):
        ...
        for component in all_song_components:
            if component in song_creation_data:
                path, name, type = _get_path_name_type_from_creation_data(song_creation_data, component)
        ...
        _db_update_song(song)
        return song

    def set(self, song, song_creation_data):
        original = _db_get_song(song['name'])
        if original:
